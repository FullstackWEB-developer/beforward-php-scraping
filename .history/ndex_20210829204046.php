...
function scraping_DB($url) {
    ...
    foreach($html->find('div.caritem_titlearea h2 a') as $a) {
        ...
        // $key = '';
        // $val = '';
//*[@id="contents_detail"]/div[3]/div[1]/div[2]/table[1]
//*[@id="contents_detail"]/div[3]/div[1]/div[1]/div/div[4]/table

        foreach($sub_html->find('ul.title h1, p.showMore a[target=_blank], [@id="contents_detail"]/div[3]/div[1]/div[2]/table[1]') as $node) {
            echo $node;
            echo "<br/>";
            // if ($node->tag=='h5')
            //     $key = $node->plaintext;
