    ...
    foreach($html->find('div.caritem_titlearea h2 a') as $a) {
        ...
        foreach($sub_html->find('ul.title h1, p.showMore a[target=_blank], div[class^=carDetails] table[class=tabA]') as $node) {
            ...
            $table = $node->find('table', 0)->find('tbody');
                ...
                $ret['Stock Id'] = $table->find('tr', 4)->find('td', 0)->plaintext;
                $ret['Inventory Location'] = $table->find('tr', 5)->find('td', 1)->plaintext;
                $ret['Stock Id'] = $table->find('tr', 6)->find('td', 0)->plaintext;
                $ret['Inventory Location'] = $table->find('tr', 7)->find('td', 1)->plaintext;
                $ret['Stock Id'] = $table->find('tr', 8)->find('td', 0)->plaintext;
                $ret['Inventory Location'] = $table->find('tr', 9)->find('td', 1)->plaintext;
                $ret['Stock Id'] = $table->find('tr', 10)->find('td', 0)->plaintext;
                $ret['Inventory Location'] = $table->find('tr', 11)->find('td', 1)->plaintext;
                $ret['Stock Id'] = $table->find('tr', 12)->find('td', 0)->plaintext;
                $ret['Inventory Location'] = $table->find('tr', 13)->find('td', 1)->plaintext;
                $ret['Stock Id'] = $table->find('tr', 14)->find('td', 0)->plaintext;
                $ret['Inventory Location'] = $table->find('tr', 15)->find('td', 1)->plaintext;
                $ret['Stock Id'] = $table->find('tr', 16)->find('td', 0)->plaintext;
                $ret['Inventory Location'] = $table->find('tr', 17)->find('td', 1)->plaintext;
                $ret['Stock Id'] = $table->find('tr', 18)->find('td', 0)->plaintext;
