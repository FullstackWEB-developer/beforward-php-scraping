    ...
    foreach($html->find('div.caritem_titlearea h2 a') as $a) {
        ...
        foreach($sub_html->find('ul.title h1, p.showMore a[target=_blank], div[class^=carDetails] table[class=tabA]') as $node) {
            ...
            $table = $node->find('table', 0)->find('tbody');
                ...
                $ret['Transmission'] = $table->find('tr', 3)->find('td', 0)->plaintext;
                $ret['Year'] = $table->find('tr', 3)->find('td', 1)->plaintext;
                $ret['Transmission'] = $table->find('tr', 4)->find('td', 0)->plaintext;
                $ret['Year'] = $table->find('tr', 4)->find('td', 1)->plaintext;
                $ret['Transmission'] = $table->find('tr', 5)->find('td', 0)->plaintext;
                $ret['Year'] = $table->find('tr', 5)->find('td', 1)->plaintext;
