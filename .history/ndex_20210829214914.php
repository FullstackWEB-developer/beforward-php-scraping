    ...
    foreach($html->find('div.caritem_titlearea h2 a') as $a) {
        ...
        foreach($sub_html->find('ul.title h1, p.showMore a[target=_blank], div[class^=carDetails] table[class=tabA]') as $node) {
            ...
            $table = $node->find('table', 0)->find('tbody');
                ...
                $ret['Transmission'] = $table->find('tr', 6)->find('td', 0)->plaintext;
                $ret['Year'] = $table->find('tr', 6)->find('td', 1)->plaintext;
                $ret['Transmission'] = $table->find('tr', 7)->find('td', 0)->plaintext;
                $ret['Year'] = $table->find('tr', 7)->find('td', 1)->plaintext;
