    ...
    foreach($html->find('div.caritem_titlearea h2 a') as $a) {
        ...
        foreach($sub_html->find('ul.title h1, p.showMore a[target=_blank], div[class^=carDetails] table[class=tabA]') as $node) {
            ...
            $table = $node->find('table', 0)->find('tbody');
                ...
                $ret['Inventory Location'] = $table->find('tr', 0)->find('td', 1)->plaintext;
                $ret['Model Code'] = $table->find('tr', 1)->find('td', 0)->plaintext;
                $ret['Year'] = $table->find('tr', 1)->find('td', 1)->plaintext;
                $ret['Transmission'] = $table->find('tr', 2)->find('td', 0)->plaintext;
                $ret['Color'] = $table->find('tr', 2)->find('td', 1)->plaintext;
                $ret['Drive'] = $table->find('tr', 3)->find('td', 0)->plaintext;
                $ret['Door'] = $table->find('tr', 3)->find('td', 1)->plaintext;
                $ret['Steering'] = $table->find('tr', 3)->find('td', 1)->plaintext;
                $ret['Seats'] = $table->find('tr', 4)->find('td', 0)->plaintext;
                $ret['Engine Type'] = $table->find('tr', 4)->find('td', 1)->plaintext;
                $ret['Body Type'] = $table->find('tr', 5)->find('td', 0)->plaintext;
                $ret['Engine Size'] = $table->find('tr', 5)->find('td', 1)->plaintext;
                $ret['Mileage'] = $table->find('tr', 6)->find('td', 0)->plaintext;
                $ret['Fuel'] = $table->find('tr', 6)->find('td', 1)->plaintext;
                $ret['Body Length'] = $table->find('tr', 7)->find('td', 0)->plaintext;
                $ret['Curb Weight'] = $table->find('tr', 7)->find('td', 1)->plaintext;
                $ret['Gross Vehicle Weight'] = $table->find('tr', 8)->find('td', 0)->plaintext;
                $ret['Max Loading Capacity'] = $table->find('tr', 8)->find('td', 1)->plaintext;
                $ret['Max Loading Capacity'] = $table->find('tr', 9)->find('td', 0)->plaintext;
